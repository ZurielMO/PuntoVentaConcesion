    ...
    <div className="grid min-h-[100dvh] lg:grid-cols-[1.1fr_1fr]">
      ...
      <aside className="dashboard-banner relative hidden flex-col justify-between overflow-hidden p-12 lg:flex xl:p-16">
        <div className="relative z-10 flex items-center gap-2">
          <span className="text-[2.2rem] font-semibold text-white">PuntoVenta</span>
          <span className="rounded-full bg-[var(--brand-yellow)] px-3 py-1 text-[1.3rem] font-semibold text-[var(--leon-black)]">
            Concesiones Estadio
          </span>
        </div>

        <div className="relative z-10 max-w-md">
          <h1 className="!text-[3.6rem] !leading-tight !text-white xl:!text-[4.2rem]">
            Panel de gestión del estadio
          </h1>
          <p className="mt-4 text-[1.7rem] leading-relaxed text-white/85">
          ...
          <ul className="mt-10 flex flex-col gap-5">
            {PANEL_FEATURES.map(({ icon: Icon, label, detail }) => (
              <li key={label} className="flex items-start gap-4">
                <span className="flex size-12 shrink-0 items-center justify-center rounded-[12px] bg-white/10 ring-1 ring-white/15">
                  <Icon className="size-6 text-[var(--brand-yellow)]" strokeWidth={2} />
                </span>
                <div>
                  <p className="text-[1.65rem] font-medium text-white">{label}</p>
                  <p className="text-[1.45rem] text-white/70">{detail}</p>
                </div>
              </li>
            ))}
          </ul>
        ...
        <p className="relative z-10 text-[1.4rem] text-white/55">
      ...
      <main className="flex items-center justify-center bg-white px-[var(--outer-gutter)] py-[var(--space-6)] lg:px-[var(--space-7)]">
        <div className="w-full max-w-[440px]">
          <div className="mb-8 flex flex-wrap items-center gap-2 lg:hidden">
            <span className="text-[2.2rem] font-semibold text-starbucks-green">PuntoVenta</span>
            <span className="rounded-full bg-[var(--brand-yellow)] px-3 py-1 text-[1.25rem] font-semibold text-[var(--leon-black)]">
              Concesiones
            </span>
          </div>

          <span className="inline-flex items-center gap-2 rounded-full bg-green-muted px-4 py-1.5 text-[1.35rem] font-medium text-green-accent ring-1 ring-green-soft">
            <ShieldCheck className="size-5" strokeWidth={2} />
            Acceso interno
          </span>

          <h2 className="mt-5 text-[3rem] font-semibold text-green-dark">Iniciar sesión</h2>
          <p className="mt-2 text-[1.55rem] text-muted-foreground">
          ...
          <p className="mt-8 text-[1.45rem] text-muted-foreground">
